...
Download weights (see section below) and then:
```bash
./run.sh
```

A new window will appear. Press `F` to toggle FPS on screen. Press `B` to enable green background.

Using green background, you can use the fake webcam generated by `run.sh` on OBS. You can then use whatever background you want on OBS using a `Chroma key compositing` filter.

Finally, you can start a virtual webcam on OBS. This webcam will be detected by Discord!
